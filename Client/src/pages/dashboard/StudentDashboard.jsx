...
const ProfileData =[
  ...
  const viewMyComplaintSection = () => {
    setActiveTab("MyComplaint");
  }

  return (
    <div className="my-5 studentDashboard mx-auto" style={{width:"100vw"}}>
        <div className="text-center head mb-4 px-4 d-flex justify-content-between align-items-center text-capitalize" style={{width:"100%"}}>
          <h4 className="text-start mx-lg-4 mx-0 my-auto fw-light" style={{ color: "#065064", width: "fit-content"}}>
          <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 640 640" width={30} height={30} fill="#065064" className="pb-1 me-1">
          ...
          <div className="btngrp d-flex justify-content-between bg-secondary bg-opacity-10 p-1 rounded-5 flex-sm-row" style={{ width:"100%",height:"fit-content"}}>              
              {
                btngrpData.map((btn, index) => (
                  <button key={index} className={`btn rounded-5 w-25 p-1 d-flex align-items-center justify-content-center border-light border-opacity-10 ${activeTab === btn.name ? 'bg-white fw-bold' : ''}`} 
                  ref={index === 0 ? buttonRef : null}
                  style={{height:"100%", fontSize:"0.9rem"}}
                  onClick={() => handlebtnClick(btn.name)}>
                    <span className="d-flex align-items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 16 16" width={20} height={20} fill="black" className={`${btn.claValue} me-1`}>
                    <path d={btn.path}/>
                    <path d={ index===1?
                    ...
                    <span className="">{btn.btnValue}</span></span></button>
                ))
              }

          </div>
            <div className="mt-4 p-0">
              {activeTab === "HomeStudent" && <HomeStudent student={student} studentComplaints={studentComplaints} changeTab={viewMyComplaintSection}/>}
              {activeTab === "MyComplaints" && <MyComplaints studentComplaints={studentComplaints}/>}
              {activeTab === "Notifications" && <Notifications />}
              {activeTab === "Profile" && 
                <div className="row text-center p-5 rounded-4 mx-1" style={{backgroundColor:"#5292a310"}}>
                  <div className="col-12">
                    <h4 className="text-start fw-light d-flex align-items-center" style={{ color: "#065064", width: "fit-content" }}>
                      ...
                      Profile</h4>
                    <img src={imgSrc} width={70} height={70} className="rounded-circle" alt="" />
                  </div>
