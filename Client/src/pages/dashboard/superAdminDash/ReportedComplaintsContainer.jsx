...
function ReportedComplaintsContainer() {
  ...
  const ReportedComplaints = allAdminsComplaints.filter(
    (complaint) => complaint.isReported && complaint.isReported === true,
  );
  ...
  const dropdowns = [
    {
      name: "Status",
      options: ["All Status", "Pending", "Resolved", "Rejected"],
    },
  ...
  const handleExport = useCallback(() => {
    if (confirm("Are you sure you want to export all complaints?")) {
      ...
      const ws = XLSX.utils.json_to_sheet(ReportedComplaints);
  ...
  const filteredComplaints = useMemo(() => {
    ...
    let complaintsToFilter = [...(ReportedComplaints || [])];
    ...
    if (searchTerm.trim() !== "") {
      ...
      complaintsToFilter = complaintsToFilter.filter(
        (c) =>
          (c.complaintTitle &&
            c.complaintTitle.toLowerCase().includes(lowerCaseSearch)) ||
          (c.complaintBody &&
            c.complaintBody.toLowerCase().includes(lowerCaseSearch)) ||
          (c.createdByEmail &&
            c.createdByEmail.toLowerCase().includes(lowerCaseSearch)) ||
          (c.createdByName &&
            c.createdByName.toLowerCase().includes(lowerCaseSearch)),
      );
    }

    complaintsToFilter.sort(
      (a, b) => new Date(b.createdAt) - new Date(a.createdAt),
    );

    return complaintsToFilter;
  }, [ReportedComplaints, filters, searchTerm]);
  ...
  return (
    <section>
      <div className="row g-2" ref={dropdownContainerRef}>
        ...
        <div className="search-input-wrapper col-lg-4 col-md-12 border-0">
          <svg
            ...
            height="16"
            fill="currentColor"
            className="bi bi-search search-icon ms-lg-1 ms-md-1"
            viewBox="0 0 16 16"
          >
                      ...
                      >
                        ...
                        {option === selectedValue && (
                          <svg
                            ...
                            height="16"
                            fill="green"
                            className="bi bi-check ms-2 bg-success border border-success bg-opacity-10 rounded-circle"
                            viewBox="0 0 16 16"
                          >
        ...
        <div className="col-lg-2 col-md-6">
          <button
            className="btn export-btn w-100"
            title="Export All Complaints as excel sheet"
      ...
      <div className="row p-2 mt-3">
        ...
        <div className="col-lg-6 col-sm-12 col-sm-12 complaintItems text-start">
          <div className="d-flex justify-content-between align-items-center mb-3">
            <h6 className="fw-normal text-start px- my-auto">
              All Reported Complaints ({filteredComplaints.length})
            </h6>
        ...
        <div
          className="col-lg-6 col-sm-12 col-sm-12 complaint-details p-5"
          style={{ zIndex: 10 }}
        >
...
export default ReportedComplaintsContainer;
